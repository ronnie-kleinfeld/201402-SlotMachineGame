...
> **Template note:** pick the marker text this repo actually uses (e.g. `TODO:`, `FIXME:`, `<name>:`) and replace `<MARKER>` below, then delete this note.

Some codebases use an inline comment marker like:

```csharp
// <MARKER>: uncomment next line, some error there
```
...
1. Search the repo for the marker, covering all comment syntaxes in use in this codebase. A single regex covering most cases: `<MARKER>\??:` - grep for this across the repo, excluding build output directories (`bin/`, `obj/`, `node_modules/`, `dist/`, etc.) and this skill's own file.
