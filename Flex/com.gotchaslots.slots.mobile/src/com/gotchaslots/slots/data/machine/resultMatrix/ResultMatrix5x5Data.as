package com.gotchaslots.slots.data.machine.resultMatrix
{
	...
	public class ResultMatrix5x5Data extends BaseResultMatirxData
	{
		// properties
		...
		// class
		public function ResultMatrix5x5Data(selectedPaylines:int, symbols:SymbolsData, paylines:BasePaylinesData)
		{
		...
		public override function IsFiveInARow(strikeValuators:Vector.<StrikeValuatorData>):Boolean
		{
			for (var i:int = 0; i < strikeValuators.length; i++)
			{
				...
				if ((strikeValuator.Payline.ID == 0 || strikeValuator.Payline.ID == 1 || strikeValuator.Payline.ID == 2 || strikeValuator.Payline.ID == 3 || strikeValuator.Payline.ID == 4) && strikeValuator.Payboxes.length == 5)
				{
					return true;
				}
			}
		...
		public override function IsFourInARow(strikeValuators:Vector.<StrikeValuatorData>):Boolean
		{
			...
			{
				var strikeValuator:StrikeValuatorData = strikeValuators[i];
				if ((strikeValuator.Payline.ID == 0 || strikeValuator.Payline.ID == 1 || strikeValuator.Payline.ID == 2 || strikeValuator.Payline.ID == 3 || strikeValuator.Payline.ID == 4) && strikeValuator.Payboxes.length == 4)
				{
					return true;
				}
			}
		...
		public override function IsColumnValuable(columnID:int):Vector.<PayboxData>
		{
			...
			if (_resultMatrix[columnID] == _resultMatrix[5 + columnID] &&
				...
				_resultMatrix[columnID] == _resultMatrix[20 + columnID])
			{
				...
				result.push(payboxes[20 + columnID]);
			}
		...
		public override function Gravity(payboxID:int):int
		{
			...
			switch (payboxID)
			{
				...
				case 24:
					...
					_resultMatrix[payboxID - 5] = _resultMatrix[payboxID - 10];
					_resultMatrix[payboxID - 10] = _resultMatrix[payboxID - 15];
					_resultMatrix[payboxID - 15] = _resultMatrix[payboxID - 20];
					_resultMatrix[payboxID - 20] = randomID;
					break;
			}
